...
class Fast2Show:
    ...
    @property
    def mtk_file(self):
        return self._mtk_path + r'\mktdt00.txt'

    @mtk_file.setter
    def mtk_file(self, value):
        self._mtk_path = value

    @property
    def fjy_file(self):
        now = datetime.now()
        name = r'\fjy{0}{1:0>2}{2:0>2}.txt'.format(now.year, now.month, now.day)
        return self._fjy_path + name

    @fjy_file.setter
    def fjy_file(self, value):
        self._fjy_path = value

    @property
    def dbf_file(self):
        return self._dbf_path + r"\show2003.dbf"

    @dbf_file.setter
    def dbf_file(self, value):
        self._dbf_path = value
        pass
    ...
    def write_mkt_to_show(self):
        ...
        for i, line in enumerate(lines):
            if i == 0:
                ...
                if i == 3:
                    self._dataMap['000000'] = self.set_first_recode(first_line, second_line[9], third_line[9], line[9])
    ...
    def fjy_to_map(self, records):
        ...
        objs[0] = my_id
        objs[1] = records[2]
        if records[6] != "" and self._jyDate < records[6]:
            objs[11] = True
        elif records[7] != "" and self._jyDate > records[7]:
            objs[11] = True
    ...
    def write_dbf(self):
        start = clock()
        keys = self._dataMap.keys()
        fields_list = [["S1", 'C', 6, 0], ["S2", 'C', 8, 0], ["S3", 'N', 8, 3],
        ...
        writer = dbfclass.DbfSseWriter(fields_list)
        recodes = [self._dataMap[key] for key in sorted(keys)]
